...
@endsection
@section('content')
<form enctype="multipart/form-data" method="POST" action="{{ route('admin.modulo.AddCorrecao', ['id' => $id])}}" class="d-flex col-12 p-0 flex-column">
    {{ csrf_field() }}
<h1 class="m-0 text-dark">Adicionar correção - Exercicio {{$nome}}</h1>
<div class="card-body">
    <div class="form-group">
        ...
        <div class="divFileName col-sm-10 text-secondary">Selecionar arquivo...</div>
            <input type="file" name="fileCorrecao" accept=".pdf" class="form-control d-none" id="inputAnexo">
    </div>
  </div>
</div>
